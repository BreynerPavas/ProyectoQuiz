...
function mostrarPregunta() {
  ...
  document.getElementById("currentQuestionText").innerText =
    peticion[actualQuestionIndex].question;
...
document.getElementById("bNext").addEventListener("click", (e) => {
  ...
  if (actualQuestionIndex >= 9) {
    // aqui tenemos que hacer la logica cuando acabemos las preguntas
...
let labels = [];
const nIntentos =
  JSON.parse(localStorage.getItem("intentosPrevios"))?.score.length || 0;
...
window.onload = () => {
  // aqui es necesario hacerlo una vez cargue
...
Array.from(circlesQuestion).forEach((circle) => {
  ...
  circle.addEventListener("click", (e) => {
    document.getElementById("exampleModalLabel").innerText =
      peticion[e.target.id].question;
    document.getElementById("correctAnswer").innerText =
      peticion[e.target.id].correct_answer;
    document.getElementById("userAnswer").innerText =
      respuestasFinales[e.target.id];
  });
});
